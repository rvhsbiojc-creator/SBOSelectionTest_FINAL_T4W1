...
function resolveImageUrl(rawRef) {
  const driveFileId = extractGoogleDriveFileId(rawRef);
  if (driveFileId) {
    return `https://drive.google.com/thumbnail?id=${encodeURIComponent(driveFileId)}&sz=w3200`;
  }
...
function extractGoogleDriveFileId(value) {
  ...
  if (filePathMatch && filePathMatch[1]) return decodeURIComponent(filePathMatch[1]);

  const idParamMatch = raw.match(/[?&]id=([^&]+)/i);
  if (idParamMatch && idParamMatch[1]) return decodeURIComponent(idParamMatch[1]);
